...
def create_classifier(architecture, labels, input_size, layers, dropout, weights=None, save_bottleneck=False):
    ...
    x = Conv2D(len(labels), (1, 1), padding='same',
               use_bias=True, strides=(1, 1), activation='softmax')(x)

    preds = GlobalAveragePooling2D()(x)
    ...
    network = Classifier(model, input_size, labels,
                         base_model.normalize, bottleneck_layer)
...
class Classifier(object):
    ...
    def save_bottleneck(self, model_path, bottleneck_layer):
        bottleneck_weights_path = os.path.join(
            os.path.dirname(model_path), 'bottleneck_weigths.h5')
    ...
    def train(self,
        ...
        train_generator, validation_generator = create_datagen(
            img_folder, valid_img_folder, batch_size, self._input_size, project_folder, augumentation, self._norm)
        model_layers, model_path = train(self._network, 'categorical_crossentropy', train_generator,
                                         validation_generator, learning_rate, nb_epoch, project_folder, first_trainable_layer, self, metrics)
